...
class Conv3dBlock(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size=3, strides=1, padding="same", **kwargs):
        ...
        # init layer variables
        self._conv3d = layer_util.conv3d(
            filters=filters,
...
class ResidualBlock(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        return self._relu(
            self._batch_norm(inputs=self._conv3d(inputs=inputs[0]), training=training)
            + inputs[1]
        )
...
class DownSampleResnetBlock(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size=3, use_pooling=True, **kwargs):
        ...
        self._residual_block = ResidualBlock(
            filters=filters, kernel_size=kernel_size, strides=1
        )
        self._max_pool3d = (
            layer_util.max_pool3d(pool_size=(2, 2, 2), strides=(2, 2, 2))
            if use_pooling
            else None
        )
        self._conv3d_block3 = (
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        h1 = (
            self._max_pool3d(inputs=r2)
            if self._use_pooling
            else self._conv3d_block3(inputs=r2, training=training)
        )
        return h1, h0
...
class DDFSummand(tf.keras.layers.Layer):
    def __init__(self, output_shape, filters=3, **kwargs):
        ...
        # init layer variables
        self._conv3d = layer_util.conv3d(
            filters=filters, strides=1, kernel_initializer="zeros"
...
class Warping(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        warped_moving_label = layer_util.warp_moving(
            moving_image_or_label=moving_label, grid_warped=grid_warped
