...
class DataLoader:
    def __init__(self, image_dir, label_dir):
        ...
        for i in range(len(image_fnames)):
            if len(images[i].shape) != 3:
                raise ValueError(
                    "The %d-th image's dimesion is not 3: %d."
                    % (i, len(images[i].shape))
                )
        if labels is not None:
            if len(image_fnames) != len(label_fnames):
                raise ValueError(
                    "The number of images (%d) and labels (%d) do not match."
                    % (len(image_fnames), len(label_fnames))
                )
            for i in range(len(image_fnames)):
                if image_fnames[i] != label_fnames[i]:
                    raise ValueError(
                        "The %d-th image and label's file name do not match: %s (image), %s (label)."
                        % (i, image_fnames[i], label_fnames[i])
                    )
                if len(labels[i].shape) not in [3, 4]:
                    raise ValueError(
                        "The %d-th label's dimesion is not 3 or 4: %d."
                        % (i, len(labels[i].shape))
                    )
                if images[i].shape != labels[i].shape[:3]:
                    raise ValueError(
                        "The %d-th image and label's shape do not match: %s (image), %s (label)."
    ...
    def load_data(dir_name):
        ...
        data = [
            np.asarray(
                nib.load(os.path.join(dir_name, fname)).dataobj, dtype=np.float32
            )
...
class PairedDataLoader:
    def __init__(
        self, moving_image_dir, fixed_image_dir, moving_label_dir, fixed_label_dir
    ):
        # sanity check
        ...
        if len(loader_moving.image_fnames) != len(loader_fixed.image_fnames):
            raise ValueError(
                "The number of moving images (%d) and fixed images (%d) do not match."
