...
def launch_registration(test: bool):
    MAIN_PATH = os.getcwd()
    ...
    learning_rate = 0.1
    total_iter = int(10) if test else int(3000)
    ...
    var_ddf = tf.Variable(
        initializer(fixed_image_size + [3]), name="ddf", trainable=True
    )
...
def main(args=None):
    ...
    launch_registration(test=args.test)
