...
images_dir = os.path.join("logs", log_dir, "test")

# Plotting results of cases 1 and 2 (testing)

plt.subplot(2, 3, 1)
label_1_fixed_image = plt.imread(os.path.join(images_dir,"label1","depth56_fixed_image.png"))
plt.imshow(label_1_fixed_image)
plt.title("Label 1 fixed image")
plt.axis("off")

plt.subplot(2, 3, 2)
label_1_fixed_label = plt.imread(os.path.join(images_dir,"label1","depth56_fixed_label.png"))
plt.imshow(label_1_fixed_label)
plt.title("Label 1 fixed label")
plt.axis("off")

plt.subplot(2, 3, 3)
label_1_predicted = plt.imread(os.path.join(images_dir,"label1","depth56_fixed_label_pred.png"))
plt.imshow(label_1_predicted)
plt.title("Label 1 prediction")
plt.axis("off")

plt.subplot(2, 3, 4)
label_1_moving_image = plt.imread(os.path.join(images_dir,"label1","depth56_moving_image.png"))
plt.imshow(label_1_moving_image)
plt.title("Label 1 moving image")
plt.axis("off")

plt.subplot(2, 3, 5)
label_1_moving_label = plt.imread(os.path.join(images_dir,"label1","depth56_moving_label.png"))
