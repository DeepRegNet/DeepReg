...
def predict_on_dataset(dataset, fixed_grid_ref, model, save_dir):
    ...
    for _, inputs_dict in enumerate(dataset):
        ...
        moving_depth = moving_image.shape[-1]
        fixed_depth = fixed_image.shape[-1]
        ...
        for sample_index in range(num_samples):
            ...
            pred_fixed_image_d = pred_fixed_image[sample_index]
            img = nib.Nifti1Image(fixed_image_d.numpy(), affine)
            nib.save(img, os.path.join(image_dir, 'fixed_image.nii.gz'))
            img = nib.Nifti1Image(moving_image_d.numpy(), affine)
            nib.save(img, os.path.join(image_dir, 'moving_image.nii.gz'))
            img = nib.Nifti1Image(pred_fixed_image_d, affine)
            nib.save(img, os.path.join(image_dir, 'pred_fixe_image.nii.gz'))
            ...
            if not os.path.exists(image_dir):
                os.makedirs(image_dir)
            for moving_depth_index in range(moving_depth):
                moving_image_d = pred_fixed_image[sample_index, ..., moving_depth_index]
                plt.imsave(
                    filename_format.format(depth_index=moving_depth_index, name="pred_fixed_image"),
                    moving_image_d, cmap='gray')  # value range for h5 and nifti might be different
                if labeled:
                    moving_label_d = pred_fixed_label[sample_index, ..., moving_depth_index]
                    plt.imsave(
                        filename_format.format(depth_index=moving_depth_index, name="pred_fixed_label"),
                        moving_label_d, vmin=0, vmax=1, cmap='gray')
