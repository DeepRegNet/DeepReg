...
for folder in folders:
    ...
    if os.path.exists(os.path.join(project_dir, data_folder, folder, 'fixed_images')) is not True:
    ...
    for sub_folder in sub_folders:
        files = os.listdir(os.path.join(project_dir, data_folder, folder,
                                        sub_folder))
        for file in files:
            if 'T1' in file:
                source = os.path.join(project_dir, data_folder, folder,
                                      sub_folder, file)
                destination = os.path.join(project_dir, data_folder, folder, 
                                           'fixed_images', file)
...
for folder in folders:
    ...
    for sub_folder in sub_folders:
        if 'Case' in sub_folder:
            shutil.rmtree(os.path.join(project_dir, data_folder, folder, 
                                       sub_folder))
    ...
    for sub_folder in sub_folders:
        ...
        for file in files:
            if 'fixed' in sub_folder:
                im_data = np.asarray(nib.load(os.path.join(project_dir, data_folder, folder,
                                        sub_folder, file)).dataobj, dtype=np.float32)
                
                im_data = ((im_data + 150) / (1200 + 150)) * 255.0  # rescale image
                
                img = nib.Nifti1Image(im_data, affine=None)
                nib.save(img, os.path.join(project_dir, data_folder, folder,
                                        sub_folder, file))
