...
from deepreg.loss.image import LocalNormalizedCrossCorrelationDEBUG
...
class RegistrationModel(tf.keras.Model):
    ...
    def build_loss(self):
        ...
        if "pred_fixed_image" in self._outputs:
            ...
            debug_value = LocalNormalizedCrossCorrelationDEBUG()(
                y_true=fixed_image, y_pred=pred_fixed_image
            )
            self.log_tensor_stats(debug_value, name="debug-lncc")
            self._build_loss(
                name="image",
