    ...
    def call(self, y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        def verify():
            for x_name, x in [
                ...
                tf.print(x_name + "_min_max", tf.reduce_min(x), tf.reduce_max(x))
                try:
...
class LocalNormalizedCrossCorrelationDEBUG(tf.keras.layers.Layer):
    """
    Local squared zero-normalized cross-correlation.
    ...
    def call(self, y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
        ...
        ncc = tf.debugging.check_numerics(
            ncc, "DEBUG LNCC ncc value NAN/INF", name="DEBUGLNCC_before_mean"
        )
        cross = tf.debugging.check_numerics(
            cross, "DEBUG LNCC ncc value NAN/INF", name="DEBUGLNCC_before_mean"
        )
